...
export default function GalleryList() {
  ...
  useEffect(() => {
    api.listGalleries()
      .then(async r => {
        r.galleries.sort((a, b) => (a.title || a.name).localeCompare(b.title || b.name));
        for (const g of r.galleries) {
          if (g.coverUrl) {
            g.coverUrl = await resolveUrl(g.coverUrl);
          }
        }
        setGalleries(r.galleries || []); setCanCreate(!!r.canCreate)
      })
